...
def capsule_neural_network(capsule_feature_size: list, input_feature: int, threshold: int, activation_function, lr: float, device: str, capsule_tall: int, capsule_wide: int, rotation_amount: int):
    ...
    def forward_layer_in_each_capsule(dataloader, layer, optimizer, layer_idx, capsule_wide_index):
        dataloader = preprocess_dataloader(dataloader, capsule_wide_index)
        bad_epoch = 0
        current_epoch = 0
        best_loss = None
    ...
    def train_capsule_column(dataloader, capsule_wide_index):
        layer_outputs = []
